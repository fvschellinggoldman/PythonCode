...
def get_segment_dicts(data_dir, dom_dict, partial=False):
    if partial:
        data_to_load = 5
    else:
        data_to_load = 10  # dummy value

    dataset_dicts = []
    for i in range(data_to_load):

        json_file = os.path.join(data_dir, str(i).zfill(6), "ground-truth.json")
...
def train_model():
    ...
    cfg.merge_from_file(model_zoo.get_config_file("COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml"))
    cfg.DATASETS.TRAIN = ("segments_train",)
    cfg.DATASETS.TEST = ()
